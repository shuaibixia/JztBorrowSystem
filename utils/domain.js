...
const RESERVATION_ACTIVE_STATUS = ['pending', 'confirmed'];
...
function validateEquipmentTransition(fromStatus, toStatus) {
  if (canTransitionEquipmentStatus(fromStatus, toStatus)) {
    return { ok: true };
  }
  return { ok: false, message: `设备状态不能从 ${fromStatus || 'unknown'} 变更为 ${toStatus || 'unknown'}` };
}
...
function normalizeEquipment(input) {
  ...
  return {
    ...
    currentHolder: item.currentHolder || item.checkedOutBy || null,
...
function hasReservationConflict(existing, startDate, endDate) {
  const start = new Date(startDate).getTime();
  const end = new Date(endDate).getTime();
  return (existing || []).some((item) => {
    if (!RESERVATION_ACTIVE_STATUS.includes(item.status)) return false;
